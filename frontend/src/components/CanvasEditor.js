...
const CanvasEditor = ({ project, fetchProjects }) => {
  ...
  // 🔹 ADDED: fixed canvas size for layout & export consistency
  const CANVAS_W = 800;
  const CANVAS_H = 600;
  ...
  // 🔹 ADDED: pass crossOrigin to avoid tainted canvas on export
  ...
  const [newShapeId, setNewShapeId] = useState(null);

  const location = useLocation();
  const [layerId, setLayerId] = useState(null);

  // ===== ADDED: login-gated download state =====
  ...
  useEffect(() => {
    const onStorage = (e) => {
      if (e.key === "access") setCanDownload(!!e.newValue);
    };
    window.addEventListener("storage", onStorage);
    return () => window.removeEventListener("storage", onStorage);
  }, []);

  // keep in sync in the same tab (storage event won't fire)
  useEffect(() => {
    const id = setInterval(() => {
      const ok = !!localStorage.getItem("access");
      setCanDownload((prev) => (prev === ok ? prev : ok));
    }, 800);
    return () => clearInterval(id);
  }, []);
  // ============================================

  // 🔹 ADDED START: download helpers (button is outside the canvas)
  ...
  const handleDownload = () => {
    // ADDED: gate for guests — open login modal via global event
    if (!canDownload) {
  ...
  }, [location.search, shapes]); // (kept as-is)
  ...
  }, [selectedId, location.pathname]); // (kept as-is)
  ...
  }, [location.search, shapes.length]); // (kept as-is)
  ...
  }, [selectedId, shapes]); // (kept as-is)

  const lId = localStorage.getItem("selectedLayerId");
  useEffect(() => {
    setSelectedId(lId);
  }, [lId]); // (kept as-is)
  ...
  useEffect(() => {
    if (selectedId && selectedId !== null) {
      ...
      console.log("shape.id === ", selectedId);
      handleSelect(selectedId);
    }
  }, [selectedId]); // (kept as-is)

  const COLOR_PALETTES = [
    ["#FF0000", "#FF7F00", "#FFFF00", "#00FF00", "#0000FF", "#4B0082", "#9400D3", "#FFFFFF", "#000000", "#808080", "#C0C0C0", "#FF69B4"],
    ["#FFD1DC", "#FFECB8", "#E2F0CB", "#B5EAD7", "#C7CEEA", "#E2D1F9", "#FFFFFF", "#F8F8F8", "#D3D3D3", "#A9A9A9", "#696969", "#FF9AA2"],
    ["#3E2723", "#5D4037", "#795548", "#8D6E63", "#A1887F", "#BCAAA4", "#D7CCC8", "#EFEBE9", "#4E342E", "#6D4C41", "#8D6E63", "#A1887F"],
  ];
  ...
  const clearCanvas = async () => {
    ...
    if (confirmed) {
      try {
        await axios.delete(`/images/${imageID}/delete-with-layers/`);
  ...
  return (
    <section className="main-container">
      <section className="canvas-editor">
        ...
        {/* MIDDLE COLUMN — canvas + external download button */}
        <div className="canvas-container">
          {/* wrapper that lets us position the button OUTSIDE the canvas */}
          <div className="stage-wrap">
            ...
            >
              <Layer>
                {bgImage && (
                  <KonvaImage image={bgImage} width={CANVAS_W} height={CANVAS_H} />
                )}
              </Layer>
            ...
            <button
              ...
              title={canDownload ? "Download" : "Login to download"}
            >
          ...
          <style jsx>{`
            ...
            .download-float:hover { opacity: 0.9; }
            .download-float:active { transform: translateY(1px); }
            .download-float:disabled {
              opacity: 0.55;
              cursor: not-allowed;
              background: #6b7280;
              border-color: #6b7280;
            ...
            .tool-sections > div { flex: 1; }

            .tool-option,
            .action-buttons-undo-redo { padding-top: 20px; }
            ...
            .tool-section { flex: 1; min-width: 150px; padding: 10px; border-radius: 6px; }

            .tool-section h3 { margin-top: 0; margin-bottom: 10px; font-size: 14px; color: #333; }
            ...
            .color-picker-container { margin-bottom: 10px; }

            .color-picker { width: 100%; height: 100px; border-radius: 6px; margin-bottom: 10px; }
            ...
            .palette-option { display: flex; align-items: center; font-size: 12px; }

            .palette-option input { margin-right: 5px; }

            .brush-size-control { font-size: 12px; }
            .brush-size-control label { display: block; margin-bottom: 5px; }
            .brush-size-control input { width: 100%; }
            ...
            .action-buttons-undo-redo { display: flex; gap: 10px; justify-content: center; }
            ...
            .action-btn.primary:hover { background: #0069d9; }
